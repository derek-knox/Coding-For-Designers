...
RGB is composed of three values. A red value, a green value, and a blue value. Combined they represent a specific color value. The color represented by a specific color value has already been decided. No work on our end. We simply rely on the various design applications we use to consistently represent colors in our digital designs. Photoshop, Illustrator, Lightroom, CSS, and Sketch are a few examples of the hundreds, if not thousands, of applications we depend on to consistently represent color.
...
You will notice the sample RGB colors above range from 0-255 for each byte’s value. If I invented the RGB code, 1-256 would be used instead. Obviously, I did not invent it. Instead we must use 0-255. Sorry, I take the blame on this one. 
