...
Hexadecimal is another common color code. It represents the same color range as RGB, just differently. Instead of a range between *0-255* for each R, G, and B value, HEX uses *00-FF* for each. The letters A-F replace the numbers 10-15 (one character instead of two). As a result, each character represents 16 different values. Naturally, each character pair represents 256 values because (16*16=256). Additionally, the *#* symbol often precedes the value. Example time.
