...
It's worth mentioning that the descriptive function names like `makeBackgroundBlack` used in this book are intentional. From the engine's perspective, a function named `x` would suffice, but a descriptive alternative is much more useful to us humans.
