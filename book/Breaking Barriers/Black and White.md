...
In either case, the underlying bits are simply a sequence of ones and zeros. The pixel count remains constant where the converted meaning determines the image's dimensions. Generally speaking, the greater the dimensions of a digital image, the more bits and thus bytes are required to represent it.
