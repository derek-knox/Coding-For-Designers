Have you ever heard of the Pareto principle? It states that roughly 80% of the output in a system comes from 20% of the input. Though this is not a hard and fast rule for all systems, it appears often enough to be a respected truth.
...
When authoring, the goal is also to grasp the program shape. If starting with no code you'll lack program shape, so this is only important when adding new code. When code already exists, this same scanning effort helps you understand where to zoom in next. As you might guess, being organized is extremely valueable when programming. Being organized helps ensure everyone wins (your curent and future self, other programmers, and end users). When adding code it's important to try to do so in the most logical place. It's almost impossible to get it right the first time and that's why we'll explore the *refactor early and often* mantra soon.
