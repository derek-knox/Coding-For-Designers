...
Spacing, tabs, and new lines do typically exist between elements however. This is to improve human readability. Imagine if the below HTML elements were all on a single line. The browser wouldn't care as the defined structure would remain the same. We would have a much tougher time reading and easily seeing the parent-child relationships.
